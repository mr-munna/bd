  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        adminEmailsSet.add('bijoymahmudmunna@gmail.com');
        if (users) {
          users.forEach(u => {
            if (u.status === 'approved' && (u.role === 'supreme_admin' || u.role === 'super_admin')) {
              if (u.email) adminEmailsSet.add(u.email);
            }
          });
        }
        const targetEmails = Array.from(adminEmailsSet);

        const submittedByName = (currentUserDoc?.displayName && currentUserDoc.displayName !== 'Unknown User' && currentUserDoc.displayName.trim() !== '') 
          ? currentUserDoc.displayName 
          : (user?.email || 'Unknown User');

        // Generate a programmatic, highly styled PDF with jsPDF
        let pdfBase64 = '';
        try {
          ...
          doc.setFontSize(22);
          doc.text("BAROBI DESIGN", 105, 15, { align: 'center' });
          doc.setFontSize(10);
          doc.text("DELIVERY CHALLAN REQUEST FOR APPROVAL", 105, 24, { align: 'center' });
          ...
          let y = 45;
          doc.setFont('helvetica', 'bold');
          doc.text("DELIVERY REQUEST DETAILS", 15, y);
          ...
          const drawMetaLine = (label: string, value: string) => {
            ...
            y += (splitVal.length * 6) + 1;
          };

          drawMetaLine("Client Name:", clientName.trim() || 'N/A');
          drawMetaLine("Site / Delivery Address:", siteAddress.trim() || 'N/A');
          drawMetaLine("Submitted By:", `${submittedByName} (${user?.email || 'N/A'})`);
          drawMetaLine("Request Time:", new Date().toLocaleString());
          drawMetaLine("Reference:", reference.trim() || 'N/A');
          drawMetaLine("Vehicle Number:", vehicleNumber.trim() || 'N/A');
          drawMetaLine("Driver Name/Phone:", `${driverName.trim() || 'N/A'} / ${driverPhone.trim() || 'N/A'}`);
          if (remark.trim()) {
            drawMetaLine("Remark:", remark.trim());
          }

          y += 5;
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(12);
          doc.text("REQUESTED PRODUCTS LIST", 15, y);
          doc.setLineWidth(0.5);
          ...
          doc.text("Size", 140, y + 5.5);
          doc.text("Quantity", 175, y + 5.5);
          y += 8;

          doc.setFont('helvetica', 'normal');
          finalItems.forEach((item, index) => {
            // Draw light border line under row
            doc.setLineWidth(0.2);
            doc.setDrawColor(241, 245, 249);
            doc.line(15, y + 8, 195, y + 8);

            doc.text(`${index + 1}`, 18, y + 5.5);
            doc.text(`${item.productName}`, 30, y + 5.5);
            doc.text(`${item.brand}`, 100, y + 5.5);
            doc.text(`${item.size || 'N/A'}`, 140, y + 5.5);
            doc.text(`${item.quantity} ${item.unit || 'pcs'}`, 175, y + 5.5);
            y += 8;
          });
          ...
          doc.text("This is an automated delivery request PDF document generated by the BAROBI DESIGN Inventory System.", 105, 285, { align: 'center' });

          pdfBase64 = doc.output('datauristring').split(',')[1];
        } catch (pdfErr) {
          console.error("Error generating delivery request PDF:", pdfErr);
        }

        const itemsSummary = finalItems.map(item => 
          `- Product: ${item.productName} (${item.brand} | ${item.productCode})
  Quantity: ${item.quantity} ${item.unit || 'pcs'}
  Size: ${item.size || 'N/A'}`
        ).join('\n\n');

        const customSubject = `[DELIVERY APPROVAL REQUEST] New Delivery Challan Clearance Pending Approval`;
        const customText = `Hi Admin,

A new Delivery Challan clearance request has been submitted and is pending your approval.

Details of the Request:
----------------------
Client Name: ${clientName.trim() || 'N/A'}
Site / Delivery Address: ${siteAddress.trim() || 'N/A'}
Submitted By: ${submittedByName} (${user?.email || 'N/A'})
Time: ${new Date().toLocaleString()}
Reference: ${reference.trim() || 'N/A'}
Remark: ${remark.trim() || 'N/A'}
Vehicle Number: ${vehicleNumber.trim() || 'N/A'}
Driver Name/Phone: ${driverName.trim() || 'N/A'} / ${driverPhone.trim() || 'N/A'}

Products Requesting Delivery Approval:
-------------------------------------
${itemsSummary}

Please review and approve this request in the Delivery Approval Center in the application. A detailed PDF summary is attached to this email.
...
Inventory Management System`;
        ...
        await fetch('/api/notify', {
          ...
          body: JSON.stringify({
            ...
            itemName: finalItems[0]?.productName || 'Delivery Request',
            notifyEmails: targetEmails,
            customSubject,
            customText,
            pdfAttachment: pdfBase64 ? {
              base64: pdfBase64,
              filename: `Delivery_Request_${clientName.trim().replace(/[^a-zA-Z0-9]/g, '_') || 'Challan'}.pdf`
            } : undefined,
            details: {
        ...
        console.error("Failed to send delivery approval notification:", notifyErr);
      }
